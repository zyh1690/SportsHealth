...
以人体部位为入口的运动康复内容平台。点选伤痛位置 → 看到对应伤病（症状 / 自测 / 康复动作 / 预防训练）→ 相关功能性训练计划。

- **差异化**：内置「动力链（Kinetic Chain）」——可追溯伤病上下游根因（如右脚踝受限 → 骨盆旋转 → 胸椎右移 → 肩胛薄弱 → 肱骨前移），而不是只看扁平症状。
- **技术栈**：uni-app（Vue3）一套代码输出 H5 / 微信小程序 / App。
- **内容源**：`content/` 下的结构化 JSON（部位 / 伤病 / 动作 / 视频 / 人群 / 计划）。
